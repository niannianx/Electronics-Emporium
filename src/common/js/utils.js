...
//获取所有省级区域数据，通过循环生成所有可能的 ID（如 0001, 0002, ..., 0099）
// 并从 tdist 中查找匹配的区域。
...
//获取某个省下的城市数据，通过传入省的 ID，
// 并根据省 ID 查找对应的城市。方法中通过获取 t 的前两位（即省的 ID），
// 然后生成所有可能的城市 ID（xx00）来匹配数据。
...
//获取某个城市下的区级数据，通过传入城市的 ID，
// 并生成区级的所有可能的 ID（xxxx）来匹配。
