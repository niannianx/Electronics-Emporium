    ...
    <div>
      <header class="category-header wrap van-hairline--bottom">
        ...
        <div class="header-search">
          ...
          <router-link tag="span" class="search-title" to="./product-list?from=category">点击搜索</router-link>
        </div>
      ...
      <div class="search-wrap" ref="searchWrap">
        <list-scroll :scroll-data="state.categoryData" class="nav-side-wrapper">
           <!-- 可以在插槽中插入具体的内容，如左侧分类列表项或右侧分类详情的展示内容 -->
          <ul class="nav-side">
            <!--循环遍历 state.categoryData 数组，为每个分类数据项渲染一个 <li> 元素。-->
            <!-- 为每个 <li> 元素提供唯一的 key，以提高 Vue 更新虚拟 DOM 的性能。 -->
            <!-- 将每个分类的名称显示在 <li> 元素中。 -->
            <!-- 根据当前选中的分类（通过 state.currentIndex 判断），动态地为 <li> 元素添加或移除 active 类，以实现选中项的样式突出显示。 -->
            <!-- 为每个 <li> 元素绑定 click 事件，当用户点击时，调用 selectMenu 方法并传递当前分类的 categoryId，从而实现分类选择的交互功能。 -->
            <li
